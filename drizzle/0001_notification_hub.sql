-- Az Notification Hub — Phase 1 Persistence Upgrade
-- Tables: notifications, notification_preferences, notification_sources,
-- notification_events_log, notification_delivery_attempts, notification_jobs, user_roles.
...
create index if not exists notification_sources_active_idx
  on public.notification_sources (tenant_id, active, source_key);
...
drop policy if exists "tenant notifications read" on public.notifications;
create policy "tenant notifications read"
  on public.notifications for select to authenticated
  using (tenant_id = coalesce(auth.jwt() ->> 'tenant_id', 'default'));

drop policy if exists "tenant notifications update" on public.notifications;
create policy "tenant notifications update"
  on public.notifications for update to authenticated
  using (tenant_id = coalesce(auth.jwt() ->> 'tenant_id', 'default'))
  with check (tenant_id = coalesce(auth.jwt() ->> 'tenant_id', 'default'));

drop policy if exists "tenant preferences read" on public.notification_preferences;
create policy "tenant preferences read"
  on public.notification_preferences for select to authenticated
  using (tenant_id = coalesce(auth.jwt() ->> 'tenant_id', 'default'));

drop policy if exists "tenant preferences write" on public.notification_preferences;
create policy "tenant preferences write"
